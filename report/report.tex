...
Within each chain there is no ordering of the elements.
...
The fixation flag is checked before growing or shrinking a table to make sure that possible split or merges won't lead into duplicate appearance of elements in a traversal.
Objects on a fixed table are not deleted if there exist no other objects with the same key.
Instead objects are marked with an ``invalid\_hash'' flag and added to a list with elements that must be deleted after the fixation has been lifted.
