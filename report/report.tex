...
All ETS tables are owned by the creating process as long as the creating process is alive and has not given the ownership to another process with the BIF \verb|ets:give_away|.
...
\section{Previous Work}
  ...
  The benchmarks in~\cite{ScottEtsJudy} does not test how the implementation performs when operations are performed concurrently.
  ...
  Benchmark results provided in~\cite{PatrikErlangTrans} for a prototype of ETS with transactional memory, shows that transactional memory support for ETS could make some use cases much more scalable with the number of cores.
...
\item Read and write operations on a table require the acquisition of the appropriate table locks.
  Depending on the type of the table, read and write operations for keys that are different can be allowed to run in parallel, by locking only parts of the table.
