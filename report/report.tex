...
If the table is of the type \verb|ordered_set| one can do it more efficiently by making use of the fact that the objects are ordered.

A table can be traversed with the help of the functions \verb|ets:first/1| and \verb|ets:next/2|.
The function \verb|ets:first/1| takes a table identifier as parameter and \verb|ets:next/2| takes a table identifier and a key.
...
One way of traversing a table of one of the hash table types without having to handle exceptions from \verb|ets:next/2| calls, would be to have the table private or at least make sure that no other process access it while it is traversed.
...
Safe traversal means that a \verb|ets:first/1| and \verb|ets:next/2| traversal of the table will not give an exception from a \verb|ets:next/2| call.
...
Safe traversal does not mean that the table can not change until it is not set to safe traversal.
