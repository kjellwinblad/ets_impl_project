...
object MultiMoves {
  ...
  def moves(array: String, nrOfWorkers: Int): Int =
    moves(array, nrOfWorkers, new ConcurrentHashMap[String, Boolean]())//(1024000, 0.7.toFloat, 32))
  ...
  abstract sealed class StringContainer
  case object Empty extends StringContainer
  case class ConsL(l: List[String], sc: StringContainer) extends StringContainer
  case class ConsC(sc1: StringContainer, sc2: StringContainer) extends StringContainer

  @tailrec
  def moves(arrays: StringContainer,
    movesSoFar: Int,
    workers: Array[Actor],
    nrOfWorkers: Int,
    ...
    if (workPackagesLeft == 0) {
      ...
      receive { case m => m } match {
        ...
        case ("work_done", newWork: StringContainer) =>
          moves(ConsC(newWork, arrays), movesSoFar, workers, nrOfWorkers, workPackagesLeft - 1, cache)
      }

  def divideWorkToWorkers(arrays: StringContainer, workers: Array[Actor], nrOfWorkers: Int, cache: AbstractMap[String, Boolean]): Unit =
    divideWorkToWorkers(arrays, workers, nrOfWorkers, 0, cache)

  @tailrec
  def divideWorkToWorkers(arrays: StringContainer, workers: Array[Actor], nrOfWorkers: Int, currentWorker: Int, cache: AbstractMap[String, Boolean]): Unit =
  ...
  def isEmpty(arrays: StringContainer) =
    (getNextAndRest(arrays) == Empty)

  @tailrec
  def getNextAndRest(l: StringContainer): StringContainer = l match {
    case Empty =>
      Empty
    case ConsL(Nil, rest) =>
      getNextAndRest(rest)
  ...
  @tailrec
  def worker(cache: AbstractMap[String, Boolean], workDoneSoFar: StringContainer, resultReciver: Actor): Unit =
    receive { case m => m } match {
      case "stop" =>
  ...
  def main(args: Array[String]) {
    ...
    time { MultiMoves.moves("bebebeeewewbewweweebe", 8) }
